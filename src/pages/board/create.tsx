...
const Create = () => {
  ...
  const [form, updateForm] = useReducer((prev: FormState, next: Partial<FormState>) => ({ ...prev, ...next }), { name: "", description: "", public: true });
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      ...
      for (const name of ["To Do", "Doing", "Done"]) {
        const column = await addDoc(collection(database, "users", user.uid, "boards", board.id, "columns"), { name, cards: [] });
        order.push(column.id);
      }

      await updateDoc(doc(database, "users", user.uid, "boards", board.id), { order });
          ...
          <form onSubmit={handleSubmit} className="flex w-full max-w-xl flex-col gap-3">
            ...
            <div className="mt-4 flex w-full items-center justify-between gap-5">
              ...
              <div className="flex items-center justify-end gap-5 ">
                <Link href="/board">
                  <button className="btn bg-gray-600 text-white hover:bg-gray-700">Cancel</button>
                </Link>
                <button type="submit" className="btn bg-primary-500 text-white hover:bg-primary-600">
                  Create
                </button>
              </div>
            </div>
            <p className={`trans my-3 text-center text-xs text-gray-400 sm:text-base ${form.public ? "opacity-100" : "opacity-0"}`}>Anyone with the link can read and write.</p>
          </form>
        </main>
      </ProtectedRoute>
    </Layout>
